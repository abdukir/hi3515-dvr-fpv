  ...
  try{
    ...
    list.innerHTML=recs.map(r=>{
      ...
      return '<div class="clip"><div class="thumb" style="background:#000 center/cover no-repeat url(/api/rec/thumb?'+q+')" onclick="playRec(\''+r.path+'\','+r.ch+')">'+
        '<span class="osd">'+(r.label||'')+'</span><span class="ch">CH 0'+(r.ch+1)+'</span>'+
        '<div class="ov"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></div></div>'+
        '<div class="info"><div class="txt"><div class="t">'+(r.label||r.file)+(r.sub?' <span style="color:var(--dim)">'+r.sub+'</span>':'')+'</div><div class="s">'+fmt(r.size)+' · '+r.file+'</div></div>'+
        '<span class="sp"></span>'+
        '<button class="rowbtn" title="play on the DVR\'s VGA monitor" onclick="playOnMonitor(\''+r.path+'\')">▶ monitor</button> '+
        '<button class="rowbtn" title="the original file straight off the DVR - instant, nothing re-encoded" onclick="location.href=\'/api/download?path='+encodeURIComponent(r.path)+'\'">↓ TS</button> '+
        '<button class="rowbtn" title="remuxed to MP4 - lossless, but it has to pull the whole recording off the device first, so a long clip takes a while" onclick="location.href=\'/api/rec/mp4?'+q+'&dl=1\'">↓ MP4</button> '+
