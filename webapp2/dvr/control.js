...
/* ["LIST 3", "R /root/rec/a1/0042_ch1_01m23s.ts 65966004 9004200000000", ...]
 *
 * Two naming schemes are understood. Current recordings are NUMBERED —
 * NNNN_chC_MMmSSs.ts — because the device clock cannot be trusted (its RTC survives
 * power-off but the kernel clock starts at 1970, so dated names sorted wrong and the
 * newest clip could hide mid-list). A _REC suffix means the recording is still being
 * written, or was cut short by a power loss. Dated names from before the change are
 * still parsed so old clips remain playable and deletable. */
function parseList(lines) {
  if (!Array.isArray(lines)) return [];
  const out = [];
  lines.forEach((l) => {
    const m = /^R (\S+) (\d+) (\S+)$/.exec(l);
    if (!m) return;
    const path = m[1];
    const file = path.split('/').pop();
    const rec = { path: path, file: file, dir: path.slice(0, path.lastIndexOf('/')),
                  size: parseInt(m[2], 10), key: m[3] };
    let f = /^(\d{4})_ch(\d)_(?:(\d{2})m(\d{2})s|(REC))\.ts$/.exec(file);
    if (f) {
      rec.seq = parseInt(f[1], 10);
      rec.ch = parseInt(f[2], 10) - 1;
      rec.partial = !!f[5];
      rec.durSec = f[5] ? null : parseInt(f[3], 10) * 60 + parseInt(f[4], 10);
      rec.label = '#' + rec.seq;
      rec.sub = f[5] ? 'incomplete' : (f[3] + 'm ' + f[4] + 's');
      out.push(rec);
      return;
    }
    f = /^(\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})_ch(\d)\.ts$/.exec(file);
    if (!f) return;
    rec.ch = parseInt(f[7], 10) - 1;
    rec.seq = null;
    rec.partial = false;
    rec.durSec = null;
    rec.date = f[1] + '-' + f[2] + '-' + f[3];
    rec.time = f[4] + ':' + f[5] + ':' + f[6];
    rec.label = rec.time;
    rec.sub = rec.date;
    out.push(rec);
  });
