...
// Clear the clip cache on startup. The previous runFfmpeg renamed SIGKILLed partials over
// the final name, so an existing cached .mp4 may be a headerless stub that would be served
// forever. Cheap to rebuild, and this guarantees nothing broken survives the fix.
try { fs.readdirSync(CACHE).forEach((f) => { try { fs.unlinkSync(path.join(CACHE, f)); } catch (e) {} }); } catch (e) {}
 ...
 *
 * Two things here were badly wrong and produced a 329 MB MP4 with no moov atom that the
 * UI happily offered for download:
 *
 *   1. A flat 60 s SIGKILL. Remuxing a 5-minute 32 Mbps recording means pulling 1.3 GB
 *      off the device over HTTP and writing it back — minutes, not seconds. The job was
 *      killed every time, long before it could write the moov.
 *   2. Success was judged by "the output is bigger than 512 bytes". A SIGKILLed ffmpeg
 *      leaves a large, headerless file, so the partial was renamed over the final name
 *      and served as if it had worked.
 *
 * Now: the exit code and signal decide success, and the timeout is a STALL timeout —
 * killed only if the output stops growing — so an honest long job runs to completion
 * while a genuinely hung one still dies. A hard ceiling remains as a backstop. */
function runFfmpeg(out, args, opts) {
  opts = opts || {};
  const stallMs = opts.stallMs || 90000;
  const hardMs = opts.hardMs || 45 * 60 * 1000;
  ...
  const pr = new Promise((resolve, reject) => {
    ...
    let err = '', killedBecause = '';
    const t0 = Date.now();
    let lastSize = -1, lastGrew = Date.now();
    const watch = setInterval(() => {
      let sz = -1;
      try { sz = fs.statSync(tmp).size; } catch (e) {}
      if (sz > lastSize) { lastSize = sz; lastGrew = Date.now(); }
      if (Date.now() - lastGrew > stallMs) killedBecause = 'stalled (no output for ' + (stallMs / 1000) + 's)';
      else if (Date.now() - t0 > hardMs) killedBecause = 'exceeded the hard time limit';
      if (killedBecause) { clearInterval(watch); try { p.kill('SIGKILL'); } catch (e) {} }
    }, 2000);
    p.stderr.on('data', (d) => { err += d; if (err.length > 8000) err = err.slice(-4000); });
    p.on('error', (e) => { clearInterval(watch); reject(e); });
    p.on('close', (code, signal) => {
      clearInterval(watch);
      const bad = killedBecause || signal || code !== 0;
      if (bad) {
        try { fs.unlinkSync(tmp); } catch (e) {}
        return reject(new Error('ffmpeg ' + (killedBecause || ('exit ' + code + (signal ? ' ' + signal : ''))) +
                                ': ' + err.slice(-300)));
      }
      if (!fs.existsSync(tmp) || fs.statSync(tmp).size <= 512) {
        try { fs.unlinkSync(tmp); } catch (e) {}
        return reject(new Error('ffmpeg produced nothing: ' + err.slice(-300)));
      }
      try { fs.renameSync(tmp, out); } catch (e) { return reject(e); }
      resolve(out);
    });
...
// Recordings are numbered now (NNNN_chC_MMmSSs.ts, or _REC while still being written);
// dated names from before that change are still accepted so old clips stay playable.
const RECPATH = /^\/root\/rec\/a[1-4]\/(?:\d{4}_ch\d_(?:\d{2}m\d{2}s|REC)|\d{8}_\d{6}_ch\d)\.ts$/;
...
const server = http.createServer(async (req, res) => {
  ...
  if (p === '/api/playback' && POST) {
    ...
    // pacing divisor: 1=1/4x 2=1/2x 4=1x 8=2x 16=4x 32=8x. Above 1x the device plays
    // keyframes only and seeks between them, so fast-forward costs constant I/O.
    else if (op === 'speed') cmd = 'PBSPEED ' + ([1, 2, 4, 8, 16, 32].indexOf(parseInt(q.v, 10)) >= 0 ? q.v : '4');
