...
/* rename(2) — recordings are written under a placeholder name and renamed once their
 * duration is known (see rec_stop in dvr.c). */
static inline long sys_rename(const char *oldp, const char *newp) {
    register long r0 asm("r0") = (long)oldp;
    register long r1 asm("r1") = (long)newp;
    asm volatile("swi %[s]" : "+r"(r0) : [s]"i"(SYS_BASE+__NR_rename), "r"(r1) : "memory");
    return r0;
}
...
/* settimeofday(2) - the kernel clock starts at 1970 on this box and nothing syncs it
 * from the RTC, so every file was stamped 1970/1980. See clock_sync() in dvr.c. */
static inline long sys_settimeofday(const struct k_timeval *tv, const void *tz) {
    register long r0 asm("r0") = (long)tv;
    register long r1 asm("r1") = (long)tz;
    asm volatile("swi %[s]" : "+r"(r0) : [s]"i"(SYS_BASE+__NR_settimeofday), "r"(r1) : "memory");
    return r0;
}
