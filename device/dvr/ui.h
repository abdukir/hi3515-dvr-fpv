...
typedef struct {
    ...
    const char *pb_names[128]; /* recording labels, newest first (see PBMAX in dvr.c) */
    int  pb_count;
...
#define PBB_SPEED 6
#define PBC_N     7   /* buttons drawn in the row */
#define PBB_BAR   7   /* the progress bar itself: also a navigable stop, ENTER = seek mode */
#define PBC_NAV   8   /* navigable positions = buttons + the bar */
...
/* speed is the pacing divisor: 4 = 1x, below = slow-mo, above = fast. seekmode highlights
 * the progress bar and shows the current jump size. */
static void ui_draw_pbbar(int paused, int ended, int speed, long cur_ms, long tot_ms, const char *title, int sel, int scrubbing, int seekmode, int seekstep){
    int scale=(g_fbw>=1000)?2:1;
    int pad=8*scale, gap=7*scale, ch=8*scale, bh=ch+pad*2;
    char sp[8]; { const char*s = (speed<=1)?"1/4x":(speed==2)?"1/2x":(speed==4)?"1x":
                               (speed==8)?"2x":(speed==16)?"4x":"8x";
                  int i=0; while(*s)sp[i++]=*s++; sp[i]=0; }
    ...
    if(seekmode){                       /* say what a left/right press will do right now */
        char s[20]; int j=0, pct=seekstep/10, frac=seekstep%10;
        s[j++]='S';s[j++]='E';s[j++]='E';s[j++]='K';s[j++]=' ';s[j++]='+';s[j++]='/';s[j++]='-';s[j++]=' ';
        if(pct>=10) s[j++]=(char)('0'+pct/10);
        s[j++]=(char)('0'+pct%10);
        if(pct<10){ s[j++]='.'; s[j++]=(char)('0'+frac); }
        s[j++]='%'; s[j]=0;
        fb_text(px+pad+fb_textw(title?title:"",scale)+8*scale, iy, s, knob, FB_TRANSPARENT, scale);
    }
    ...
    { int kx=prx+fw-2*scale, kw=4*scale; if(kx<prx)kx=prx; if(kx>prx+prw-kw)kx=prx+prw-kw;
      fb_rect(kx, iy-3*scale, kw, prg_h+6*scale, (scrubbing||seekmode)?knob:fg); }   /* scrub knob */
    /* focused / in seek mode: outline the bar so it reads as a control, not just a gauge */
    if(sel==PBB_BAR) fb_border(prx-2*scale, iy-4*scale, prw+4*scale, prg_h+8*scale, scale,
                               seekmode?knob:selbg);
    iy += prg_h + 12*scale;
