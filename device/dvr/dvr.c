...
/* Playback pacing divisor. 4 = 1x; SMALLER is slow-motion, LARGER is fast-forward:
 * 1 = 1/4x, 2 = 1/2x, 4 = 1x, 8 = 2x, 16 = 4x, 32 = 8x. Above 1x we play KEYFRAMES
 * ONLY -- decoding 8x30 fps is far beyond this VDEC, but an IDR every ~0.5 s is not,
 * and each one is independently decodable so the picture stays clean. */
static int      g_pb_speed = 4;
#define PB_FF_BEAT_MS 200   /* fast-forward shows ~5 pictures a second at every speed */
static int      g_pb_seekmode = 0;    /* progress bar focused AND armed: -/+ jump */
static unsigned g_pb_seek_t   = 0;    /* now_ms() of the last jump, for acceleration */
static int      g_pb_seek_step = 10;  /* current jump size, permille of the file */
...
static long file_size(const char *path);   /* fwd: rec_start probes for a free number */
...
/* dvr.conf `capture=`: 0 = one field (704x240 NTSC / 704x288 PAL, 30/25 fps and
 * progressive - the proven default), 1 = both fields woven into a full-height frame
 * (704x480 / 704x576). Both fields double the vertical detail but weave two exposures
 * 1/60 s apart, so fast motion combs - and FPV is nothing but fast motion. Off unless
 * you ask for it, and re-measure the frame rate if you turn it on: it doubles the
 * pixels the encoder and the disk have to carry. */
static int g_capboth = 0;
...
static unsigned g_max_rec_mb  = 1800;   /* cap one recording file. NOT about the vfat 4 GB
                                         * limit: sys_lseek here is 32-bit, so playback of a
                                         * file past 2047 MiB reads a negative size. */
...
static void apply_cfg(void){
    ...
    while(i<n){
        ...
        else if(streq(key,"capture")&&val>=0&&val<=1) g_capboth=val;
        ...
        else if(streq(key,"max_rec_mb")&&val>0) g_max_rec_mb=(val>1900)?1900:(unsigned)val;
...
/* What the TW2866 itself says it is seeing, per channel.
 *
 * Register ch*0x10+0x00 is the decoder's status byte:
 *   bit7 VDLOSS (1 = no signal)   bit6 HLOCK   bit5 SLOCK
 *   bit4 FLD    bit3 VLOCK        bit2 MONO    bit1 DET50
 * DET50 is the 50 Hz flag, i.e. PAL/SECAM. Auto-detect used to read bit 0 instead,
 * which is not the standard bit at all and reads 0 in practice - so "AUTO" could only
 * ever resolve to NTSC no matter what was plugged in. Verified against the live chip:
 * with no camera it reads 0x96 (VDLOSS=1, DET50=1) while bit0 = 0.
 *
 * Returns 1 = NTSC, 2 = PAL, 0 = nothing locked. Only a channel with VDLOSS clear
 * counts: DET50 means nothing while the decoder has no signal to measure. */
static int tw_detect(int ftw, int *lockmask){
    int c, votes_pal = 0, votes_ntsc = 0, mask = 0;
    if(ftw < 0){ if(lockmask) *lockmask = 0; return 0; }
    for(c=0;c<NCH;c++){
        int v = tw_read(ftw, c*0x10 + 0x00);
        if(v < 0 || (v & 0x80)) continue;                 /* VDLOSS: no signal on this one */
        mask |= (1<<c);
        if(v & 0x02) votes_pal++; else votes_ntsc++;      /* DET50 */
    }
    if(lockmask) *lockmask = mask;
    if(!mask) return 0;
    return votes_pal > votes_ntsc ? 2 : 1;
}
...
static int resolve_standard(int ftw){
    ...
    int mask = 0, std = tw_detect(ftw, &mask);
    puts_("[dvr] auto-detect locked="); puthex((unsigned)mask);
    puts_(" -> "); puts_(std==2?"PAL\n":(std==1?"NTSC\n":"nothing (defaulting NTSC)\n"));
    return std ? std : 1;
}
...
static void apply_standard(int ftw){
    ...
    { unsigned fh = (std==2)?0x120u:0xf0u;           /* one field: 288 / 240 */
      put32(VI_CHN, 12, g_capboth ? fh*2u : fh);     /* both fields: 576 / 480 */
      put32(VI_CHN, 16, g_capboth ? 2u : 1u); }      /* enCapSel: 2 = BOTH, 1 = BOTTOM */
    put32(VGA_LAYER, 20, (std==2)?576:480);          /* VO layer source height (field doubled) */
    if(std==2){
        g_fps = 25;                                  /* PAL = 25 fps (NTSC keeps configured fps) */
        /* The VI capture rate comes from the FIELD rate, so it has to move as well:
         * asking a 50 Hz source for 60 (or 30) fields a second is a rate mismatch on the
         * one path that must stay in lockstep with the analog timing. Scaled rather than
         * hardcoded, so a low-latency vifps=60 becomes 50 instead of collapsing to 25. */
        g_vifps = (g_vifps >= 60) ? 50 : 25;
    }
    g_eheight = (std==2)?288:240;                    /* encode the single-field height */
    if(g_capboth) g_eheight *= 2;                    /* ...or the full woven frame */
    patch_venc_attrs();                              /* re-derive VENC w/h/fps/gop/bitrate/bufsz */
    puts_("[dvr] standard="); puts_(std==2?"PAL":"NTSC");
    puts_(g_capboth?" capture=both-fields\n":" capture=field\n");
}
...
static unsigned rec_seq[NCH];       /* this recording's sequence number (filename + sort key) */
static unsigned long long rec_last_us[NCH];  /* newest encoder stamp written, for the duration */
static unsigned g_seq_next = 0;     /* next free number; 0 = not scanned yet */
...
/* ---- recording names -------------------------------------------------------
 * Files are NUMBERED, not dated: NNNN_chC_MMmSSs.ts.
 *
 * The clock is not trustworthy on this box. The RTC survives power-off but the kernel
 * clock starts at 1970 and nothing syncs it, so recordings carried a sensible-looking
 * name and a 1980 mtime; and if the RTC is ever wrong or reset, dated names sort into
 * the wrong order and the newest recording hides in the middle of the list. A counter
 * cannot drift. It also sorts lexicographically, so "largest number" == "newest" with
 * no date parsing anywhere.
 *
 * The duration is in the name because that is the one thing you actually want when
 * picking a clip out of a list, and it costs nothing to put there. It is only known
 * when recording STOPS, so the file is written as NNNN_chC_REC.ts and renamed on close.
 * A leftover _REC file therefore means "this recording was cut short" — which is worth
 * seeing rather than hiding. */
static int u2s_pad(char *b, unsigned v, int w){
    int i; for(i=w-1;i>=0;i--){ b[i]=(char)('0'+(v%10)); v/=10; }
    return w;
}
/* dur < 0 -> the in-progress "_REC" name */
/* days since 1970-01-01 from a civil date (proleptic Gregorian, UTC) */
...
/* one-shot: kernel clock := RTC. Silently does nothing if the RTC looks implausible. */
static void kclock_from_rtc(void){
    struct rtc_time_ t;
    if(rtc_read(&t) != 0) return;
    if(t.tm_year < 100 || t.tm_year > 200) return;        /* not a believable year */
    long days = days_from_civil(1900 + t.tm_year, t.tm_mon + 1, t.tm_mday);
    struct k_timeval tv;
    tv.tv_sec  = days*86400L + t.tm_hour*3600L + t.tm_min*60L + t.tm_sec;
    tv.tv_usec = 0;
    if(sys_settimeofday(&tv, 0) == 0){
        puts_("[dvr] clock set from RTC: "); putu((unsigned)(1900+t.tm_year));
        puts_("-"); putu((unsigned)(t.tm_mon+1)); puts_("-"); putu((unsigned)t.tm_mday);
        puts_(" "); putu((unsigned)t.tm_hour); puts_(":"); putu((unsigned)t.tm_min);
        puts_("\n"); }
    else puts_("[dvr] clock sync failed (files keep 1970 dates)\n");
}
static void rec_name(char *nm, int chn, unsigned seq, long dur){
    int p=0;
    memcpy(nm,"/root/rec/a",11); p=11; nm[p++]=(char)('1'+chn); nm[p++]='/';
    p += u2s_pad(nm+p, seq, 4);
    memcpy(nm+p,"_ch",3); p+=3; nm[p++]=(char)('1'+chn); nm[p++]='_';
    if(dur < 0){ memcpy(nm+p,"REC",3); p+=3; }
    else { unsigned mm=(unsigned)(dur/60), ss=(unsigned)(dur%60);
           if(mm>99){ mm=99; ss=59; }
           p += u2s_pad(nm+p, mm, 2); nm[p++]='m';
           p += u2s_pad(nm+p, ss, 2); nm[p++]='s'; }
    memcpy(nm+p,".ts",4);            /* includes the NUL */
}
/* highest NNNN already on any partition, so numbering survives a restart */
static unsigned seq_scan_max(void){
    unsigned best=0; int p;
    for(p=0;p<NCH;p++){
        ...
        while((n=sys_getdents64(fd, buf, sizeof(buf)))>0){
            ...
            while(off<n){
                ...
                if(reclen==0) break;
                char *nmm=(char*)(e+19);
                if(nmm[0]>='0'&&nmm[0]<='9' && nmm[1]>='0'&&nmm[1]<='9' &&
                   nmm[2]>='0'&&nmm[2]<='9' && nmm[3]>='0'&&nmm[3]<='9' && nmm[4]=='_'){
                    unsigned v=(unsigned)((nmm[0]-'0')*1000+(nmm[1]-'0')*100+
                                          (nmm[2]-'0')*10+(nmm[3]-'0'));
                    if(v>best) best=v;
                }
                off += reclen;
...
static void rec_start(int chn){
    ...
    char nm[64]; int guard;
    if(!g_seq_next) g_seq_next = seq_scan_max() + 1;
    for(guard=0; guard<10000; guard++){        /* never reuse a number still on disk */
        rec_name(nm, chn, g_seq_next, -1);
        if(file_size(nm) < 0) break;
        g_seq_next++;
    }
    rec_seq[chn] = g_seq_next++;
    ...
    rec_base_us[chn]=0; rec_last_us[chn]=0; rec_stopping[chn]=0;   /* base latched on the first frame written */
...
static void rec_stop(int chn){
    ...
    /* Put the real duration in the name. It comes from the encoder's own timestamps,
     * not the clock, so it is right even though the system clock never gets set. */
    { char oldn[64], newn[64]; long dur = 0;
      if(rec_last_us[chn] > rec_base_us[chn])
          dur = (long)((rec_last_us[chn] - rec_base_us[chn]) / 1000000ull);
      rec_name(oldn, chn, rec_seq[chn], -1);
      rec_name(newn, chn, rec_seq[chn], dur);
      if(sys_rename(oldn, newn) != 0)
          puts_("[dvr] REC rename failed (kept the _REC name)\n"); }
...
/* Insert one recording into the list, kept sorted newest-first, capped at PBMAX.
 *
 * This used to collect the first PBMAX entries in DIRECTORY order and only then sort
 * them, which silently threw away exactly the recordings you wanted: new files land at
 * the end of the vfat directory, so once there were more than PBMAX on disk the newest
 * ones were cut before the sort ever ran and never appeared in Playback or LIST.
 * Insert-in-order instead, so the cap drops the OLDEST rather than the newest. */
static int pb_cnt = 0;
static void pb_insert(const char *ky, const char *dir, const char *name, const char *lb){
    int pos, i, z, c;
    for(pos=0; pos<pb_cnt; pos++){
        c = 0;
        for(z=0; z<15; z++){ if(ky[z]!=pb_key[pos][z]){ c = (ky[z]>pb_key[pos][z])?1:-1; break; } }
        if(c > 0) break;
    }
    if(pos >= PBMAX) return;                       /* older than everything we hold */
    { int last = (pb_cnt < PBMAX) ? pb_cnt : PBMAX-1;
      for(i=last; i>pos; i--){
          memcpy(pb_key[i],   pb_key[i-1],   16);
          memcpy(pb_path[i],  pb_path[i-1],  48);
          memcpy(pb_label[i], pb_label[i-1], 20);
      } }
    memcpy(pb_key[pos], ky, 15); pb_key[pos][15]=0;
    { char *fp=pb_path[pos]; int j=0; while(dir[j]){ fp[j]=dir[j]; j++; }
      fp[j++]='/'; { int m=0; while(name[m] && j<47) fp[j++]=name[m++]; } fp[j]=0; }
    { char *d=pb_label[pos]; int j=0; while(lb[j] && j<19){ d[j]=lb[j]; j++; } d[j]=0; }
    if(pb_cnt < PBMAX) pb_cnt++;
}
 ...
 * Understands both naming schemes: the current NNNN_chC_MMmSSs.ts and the older dated
 * YYYYMMDD_HHMMSS_chN.ts, so recordings made before the change are still listed. Keys are
 * 15 chars and compared as text; numbered files start '9' and dated ones '2', so numbered
 * recordings always sort above the legacy ones. */
    ...
    for(p=0; p<NCH; p++){
        ...
        while((n=sys_getdents64(fd, buf, sizeof(buf)))>0){
            ...
            while(off<n){
                ...
                if(!(L>=12 && name[L-3]=='.' && name[L-2]=='t' && name[L-1]=='s')){ off+=reclen; continue; }
                char ky[16], lb[20]; int j=0, m;
                int dig4 = (name[0]>='0'&&name[0]<='9' && name[1]>='0'&&name[1]<='9' &&
                            name[2]>='0'&&name[2]<='9' && name[3]>='0'&&name[3]<='9');
                if(dig4 && name[4]=='_' && name[5]=='c' && name[6]=='h'){
                    /* NNNN_chC_MMmSSs.ts */
                    ky[0]='9'; for(m=0;m<4;m++) ky[1+m]=name[m];
                    for(m=5;m<15;m++) ky[m]='0';
                    lb[j++]='#';
                    { int q=0; while(q<3 && name[q]=='0') q++;      /* trim leading zeros */
                      while(q<4) lb[j++]=name[q++]; }
                    lb[j++]=' ';
                    for(m=8+1; m<L-3 && j<15; m++) lb[j++]=name[m]; /* duration or REC */
                    lb[j++]=' '; lb[j++]='C'; lb[j++]='H';
                    lb[j++]=(name[7]>='1'&&name[7]<='4')?name[7]:'?';
                    lb[j]=0;
                } else if(L>=22 && dig4 && name[8]=='_'){
                    /* legacy YYYYMMDD_HHMMSS_chN.ts */
                    ky[0]='2'; { int ki=1; for(m=0;m<15 && name[m];m++) if(m!=8) ky[ki++]=name[m]; }
                ...
                } else { off += reclen; continue; }
                pb_insert(ky, dpath, name, lb);
                off += reclen;
            }
        }
        sys_close(fd);
...
/* ------------------------------------------------------------------ egg music
 * Each trigger of the egg plays the NEXT tune in order, so you hear the whole set
 * before any repeats rather than rolling dice and getting the same one twice.
 *
 * All of these are public domain. They are also chosen for one physical reason: a
 * one-bit buzzer has NO timbre, so what survives it is wide leaps, a hard rhythm, few
 * distinct pitches and everything inside roughly 800-2500 Hz where this transducer is
 * actually loud. Narrow stepwise motion, which is most of what a wind march is made of,
 * just warbles -- that is why the Hucum Marsi excerpt below disappoints even though it is
 * a faithful transcription. Nothing was mis-transcribed; it is the wrong KIND of tune for
 * the hardware. Bear that in mind before adding one.
 *
 * Every note is one 90 ms unit and a SUSTAINED note is that unit REPEATED. Not stylistic:
 * buzz_tone() bit-bangs the GPIO and blocks for the note's whole length, and egg_tick()
 * only moves the plane between notes, so note length IS the animation's frame rate. Never
 * lengthen a note here -- add another unit. Regenerate any of these with tools/mel2c.py,
 * which takes note names or a MIDI file:
 *   py -3 tools/mel2c.py - --beat 2 --max-sec 9 --octave 1 --name MEL_X < tools/tunes/X.txt
 */

/* Korobeiniki -- Russian folk, 1860s. The Tetris theme: the most widely heard
 * melody ever played through a one-bit beeper, so it is proven on hardware like this. */
static const note_t MEL_TETRIS[] = {
    {1319,90},{1319,90},{1319,90},{1319,90},
    { 988,90},{ 988,90},{1047,90},{1047,90},
    {1175,90},{1175,90},{1175,90},{1175,90},
    {1047,90},{1047,90},{ 988,90},{ 988,90},
    { 880,90},{ 880,90},{ 880,90},{ 880,90},
    { 880,90},{ 880,90},{1047,90},{1047,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1175,90},{1175,90},{1047,90},{1047,90},
    { 988,90},{ 988,90},{ 988,90},{ 988,90},
    { 988,90},{ 988,90},{1047,90},{1047,90},
    {1175,90},{1175,90},{1175,90},{1175,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1047,90},{1047,90},{1047,90},{1047,90},
    { 880,90},{ 880,90},{ 880,90},{ 880,90},
    { 880,90},{ 880,90},{ 880,90},{ 880,90},
    { 880,90},{ 880,90},{ 880,90},{ 880,90},
    {1175,90},{1175,90},{1175,90},{1175,90},
    {1175,90},{1175,90},{1397,90},{1397,90},
    {1760,90},{1760,90},{1760,90},{1760,90},
    {1568,90},{1568,90},{1397,90},{1397,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1319,90},{1319,90},{1047,90},{1047,90}
};
#define MEL_TETRIS_N (int)(sizeof(MEL_TETRIS)/sizeof(MEL_TETRIS[0]))

/* Beethoven, Ode to Joy (9th, 1824). Stepwise, but slow enough that each pitch
 * gets time to register, and everyone on earth can name it. */
static const note_t MEL_ODE[] = {
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1397,90},{1397,90},{1397,90},{1397,90},
    {1568,90},{1568,90},{1568,90},{1568,90},
    {1568,90},{1568,90},{1568,90},{1568,90},
    {1397,90},{1397,90},{1397,90},{1397,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1175,90},{1175,90},{1175,90},{1175,90},
    ...
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1319,90},{1319,90},{1175,90},{1175,90},
    {1175,90},{1175,90},{1175,90},{1175,90},
    {1175,90},{1175,90},{1175,90},{1175,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
    {1319,90},{1319,90},{1319,90},{1319,90},
...
#define MEL_ODE_N (int)(sizeof(MEL_ODE)/sizeof(MEL_ODE[0]))

/* Beethoven, Fuer Elise (1810). The opening alternation is two pitches a semitone
 * apart at speed, which is the one narrow figure a square wave renders WELL -- it
 * reads as a trill rather than as mush. */
static const note_t MEL_ELISE[] = {
    {1319,90},{1319,90},{1245,90},{1245,90},
    {1319,90},{1319,90},{1245,90},{1245,90},
    {1319,90},{1319,90},{ 988,90},{ 988,90},
    {1175,90},{1175,90},{1047,90},{1047,90},
    { 880,90},{ 880,90},{ 880,90},{ 880,90},
    { 880,90},{ 880,90},{0,90},{0,90},
    {1047,90},{1047,90},{1319,90},{1319,90},
    {1760,90},{1760,90},{1976,90},{1976,90},
    {1976,90},{1976,90},{1976,90},{1976,90},
    {0,90},{0,90},{1319,90},{1319,90},
    {1661,90},{1661,90},{1976,90},{1976,90},
    {2093,90},{2093,90},{2093,90},{2093,90},
    {2093,90},{2093,90},{0,90},{0,90},
    {1319,90},{1319,90},{1245,90},{1245,90},
    {1319,90},{1319,90},{1245,90},{1245,90},
    {1319,90},{1319,90},{ 988,90},{ 988,90},
    {1175,90},{1175,90},{1047,90},{1047,90},
...
#define MEL_ELISE_N (int)(sizeof(MEL_ELISE)/sizeof(MEL_ELISE[0]))

/* Hucum Marsi -- traditional Ottoman mehter march, the fast strain (from the MIDI,
 * not from memory). Kept because it is the one that was asked for first; it fares
 * worst here for the reason in the header, but at 1-in-5 it is a guest, not the act. */
static const note_t MEL_MARSI[] = {
    { 988,90},{ 988,90},{ 988,90},{ 988,90},
    { 988,90},{ 988,90},{ 988,90},{ 988,90},
    { 988,90},{ 988,90},{ 988,90},{ 880,90},
    { 988,90},{ 988,90},{ 988,90},{ 880,90},
    { 988,90},{ 988,90},{1109,90},{ 988,90},
    {1109,90},{ 988,90},{ 880,90},{ 880,90},
    { 880,90},{ 880,90},{ 880,90},{ 831,90},
    { 880,90},{ 880,90},{ 880,90},{ 831,90},
    { 880,90},{ 880,90},{ 988,90},{ 880,90},
    { 988,90},{ 880,90},{ 831,90},{ 831,90},
    { 988,90},{ 988,90},{ 988,90},{ 880,90},
    { 988,90},{ 988,90},{ 988,90},{ 880,90},
    { 988,90},{ 988,90},{1109,90},{ 988,90},
    {1109,90},{ 988,90},{ 880,90},{ 880,90},
    ...
    { 880,90},{ 880,90},{ 988,90},{ 880,90},
    { 988,90},{ 880,90},{ 831,90},{ 831,90},
    { 740,90},{ 740,90},{ 740,90},{ 659,90},
    { 740,90},{ 740,90},{ 831,90},{ 831,90},
    { 880,90},{ 880,90},{ 880,90},{ 988,90},
    { 880,90},{ 880,90},{ 831,90},{ 831,90}
};
#define MEL_MARSI_N (int)(sizeof(MEL_MARSI)/sizeof(MEL_MARSI[0]))

/* Local, UNPUBLISHED tunes. device/dvr/tunes_local.h is gitignored, and tools/publish.sh
 * mirrors via `git ls-files`, so nothing in it can reach either repo even by accident.
 * That separation is the whole point: a melody still in copyright is fine on your own
 * buzzer and not fine committed to a public mirror. The header defines its own note_t
 * tables (generate them with tools/mel2c.py from a MIDI) and then one line:
 *   #define EGG_LOCAL_TUNES { "RICKROLL  ", MEL_RICK, MEL_RICK_N },
 * If the file is absent -- as it is in a fresh clone -- nothing here changes.
 */
...
/* The rotation. Name is for the log only -- the caption on screen stays the caption. */
typedef struct { const char *name; const note_t *m; int n; } egg_tune_t;
static const egg_tune_t EGG_TUNES[] = {
    { "KOROBEINIKI  ", MEL_TETRIS , MEL_TETRIS_N },
    { "ODE TO JOY   ", MEL_ODE    , MEL_ODE_N },
    { "FUR ELISE    ", MEL_ELISE  , MEL_ELISE_N },
    { "HUCUM MARSI  ", MEL_MARSI  , MEL_MARSI_N },
#ifdef EGG_LOCAL_TUNES
    EGG_LOCAL_TUNES                     /* never committed; see above */
#endif
};
#define EGG_NTUNES (int)(sizeof(EGG_TUNES)/sizeof(EGG_TUNES[0]))
static int g_egg_tune = 0;              /* advances only when a tune actually plays */
...
static void egg_start(void){
    ...
    if(g_ui_snd){
        /* Next tune in the rotation, and only advance when one really played -- with
         * beeps off nothing is heard, so nothing should be consumed either. */
        const egg_tune_t *t = &EGG_TUNES[g_egg_tune];
        g_egg_tune = (g_egg_tune + 1) % EGG_NTUNES;
        melody(t->m, t->n);
        puts_("[dvr] egg - clear the runway: "); puts_(t->name); puts_("\n");
    } else puts_("[dvr] egg - clear the runway (silent)\n");
}
...
static void pb_activate(int idx){
    ...
    else if(idx==PBB_SPEED){        /* 1/4x -> 1/2x -> 1x -> 2x -> 4x -> 8x -> wrap */
        g_pb_speed = (g_pb_speed>=32) ? 1 : g_pb_speed*2;
    }
    else if(idx==PBB_BAR){          /* arm/disarm jump-by-arrow on the progress bar */
        g_pb_seekmode = !g_pb_seekmode;
        g_pb_seek_step = 10; g_pb_seek_t = 0;
        if(g_pb_seekmode) g_pb_paused = 1;   /* you are inspecting, not watching */
    }
    pb_bar_draw(0);
}
/* physical-button / MCU keys during playback: -/+ move the highlight, M activates, X exits */
/* Jump by one step in seek mode. Repeated presses inside PB_SEEK_ACCEL_MS double the step,
 * so a tap nudges and a held-down burst crosses the clip; it resets after a pause. */
#define PB_SEEK_ACCEL_MS 700
#define PB_SEEK_MIN  5      /* 0.5% of the file */
#define PB_SEEK_MAX  100    /* 10% */
static void pb_seek_by(int dir){
    unsigned now = now_ms();
    if(g_pb_seek_t && (now - g_pb_seek_t) < PB_SEEK_ACCEL_MS){
        g_pb_seek_step *= 2;
        if(g_pb_seek_step > PB_SEEK_MAX) g_pb_seek_step = PB_SEEK_MAX;
    } else g_pb_seek_step = PB_SEEK_MIN*2;      /* a fresh press starts at 1% */
    g_pb_seek_t = now;
    { int cur = (g_pb_total>0) ? (int)((long long)g_pb_pos*1000/g_pb_total) : 0;
      cur += dir * g_pb_seek_step;
      if(cur<0) cur=0; if(cur>1000) cur=1000;
      g_pb_seek_pm = cur; g_pb_seek = 1; g_pb_paused = 1; }
}
/* physical-button / MCU keys during playback: -/+ move the highlight, M activates, X exits.
 * With the progress bar armed (M on it), -/+ jump through the clip instead of moving the
 * highlight, and M or X hands navigation back. */
static void pb_btnkey(char k){
    g_pb_lastui = now_ms();
    if(g_pb_seekmode){
        if(k=='-'){ pb_seek_by(-1); pb_bar_draw(0); return; }
        if(k=='+'){ pb_seek_by(+1); pb_bar_draw(0); return; }
        if(k=='M' || k=='X'){ g_pb_seekmode=0; pb_bar_draw(0); return; }
    }
    if(k=='-'){ g_pb_sel=(g_pb_sel==0)?PBC_NAV-1:g_pb_sel-1; pb_bar_draw(0); }
    else if(k=='+'){ g_pb_sel=(g_pb_sel+1)%PBC_NAV; pb_bar_draw(0); }
...
static void ctl_exec(char *line, int rfd){
    ...
    if(tok_eq(line,"PBSPEED")){ const char *a=tok_next(line);   /* PBSPEED <4|2|1> = 1x/half/quarter */
        int v=k_atoi(a);       /* 1=1/4x 2=1/2x 4=1x 8=2x 16=4x 32=8x */
        g_pb_speed=(v==1||v==2||v==4||v==8||v==16||v==32)?v:4;
        wrs(rfd,"OK\n"); return; }
    ...
    if(tok_eq(line,"UITEST")){                          /* draw a full-screen test OSD + dump fb */
        ...
        fb_dump("/root/rec/a1/fb.raw");   /* SATA, not the RAM disk: fb0 is 2.6 MB and
                                          * /root/rec has ~1.9 MB free, so the old path
    ...
    if(tok_eq(line,"TWSTD")){   /* per-channel capture-chip status - why is there no picture? */
        char m[160]; int l=0, c2;
        memcpy(m,"TWSTD",5); l=5;
        for(c2=0;c2<NCH;c2++){
            int v = (g_ftw>=0) ? tw_read(g_ftw, c2*0x10 + 0x00) : -1;
            m[l++]=' '; m[l++]='c'; m[l++]='h'; m[l++]=(char)('1'+c2); m[l++]='=';
            if(v < 0){ m[l++]='?'; continue; }
            if(v & 0x80){ memcpy(m+l,"nosignal",8); l+=8; }
            else {
                const char *sd = (v & 0x02) ? "PAL" : "NTSC";
                int k=0; while(sd[k]) m[l++]=sd[k++];
                m[l++]='/';
                m[l++]=(v&0x40)?'H':'-';      /* HLOCK  */
                m[l++]=(v&0x20)?'S':'-';      /* SLOCK  */
                m[l++]=(v&0x08)?'V':'-';      /* VLOCK  */
        ...
        memcpy(m+l," using=",7); l+=7;
        { const char *u = g_standard==2?"PAL":(g_standard==1?"NTSC":"AUTO"); int k=0; while(u[k]) m[l++]=u[k++]; }
        m[l++]='\n'; if(rfd>=0) sys_write(rfd,m,l); return;
    }
    ...
    if(tok_eq(line,"INFO")){
        ...
        { int lm=0, dt=tw_detect(g_ftw,&lm);          /* what the capture chip actually sees */
          ADDS(" det="); ADDS(dt==2?"PAL":(dt==1?"NTSC":"none"));
          ADDS(" lock="); ADDU((unsigned)lm); }
        ...
        if(g_pb_active){   /* position/duration in seconds + the pacing divisor, so a client
                            * (or a test) can see playback progress without the screen */
            ADDS(" pbs="); ADDU((unsigned)(g_pb_cur_ms/1000));
            b[l++]=(char)(47); ADDU((unsigned)(g_pb_tot_ms/1000));
            ADDS(" pbspd="); ADDU((unsigned)g_pb_speed);
            ADDS(" pbseek="); b[l++]=(char)(48+(g_pb_seekmode?1:0));
        }
...
static void pb_bar_draw(int full){
    ...
    if(show) ui_draw_pbbar(g_pb_paused, g_pb_ended, g_pb_speed, g_pb_cur_ms, g_pb_tot_ms, g_pb_title, g_pb_sel, g_pb_scrub, g_pb_seekmode, g_pb_seek_step);
...
/* ---- display aspect: rewrite the SPS as it leaves the encoder ----------------
 * The VENC emits an SPS with no VUI, so nothing tells a player the picture is 4:3.
 * Every recording and the live stream therefore rendered 704x240 as a 2.93:1 slot —
 * badly stretched. sps.h inserts an Extended_SAR; this caches the result, because the
 * encoder emits a byte-identical SPS on every keyframe and the rewrite is bit-level.
 * The replacement carries its own 4-byte start code so it can be emitted in one go. */
static unsigned char g_spsfix[4 + SPS_MAXOUT] = { 0,0,0,1 };
static unsigned      g_spsfix_len = 0;          /* incl. the start code; 0 = unusable */
static unsigned char g_spssrc[SPS_MAXIN];
static unsigned      g_spssrc_len = 0;
static int           g_sps_logged = 0;

/* If this pack opens with an SPS NAL, return the patched replacement's length and set
 * *skip to where the untouched remainder of the pack resumes. 0 = emit it verbatim. */
static unsigned sps_split(const unsigned char *d, unsigned len, unsigned *skip){
    unsigned i, end = 0, nlen, n;
    if(len < 9) return 0;
    if(d[0]||d[1]||d[2]||d[3]!=1) return 0;      /* want a 4-byte start code */
    if((d[4]&0x1f) != 7) return 0;               /* ...followed by an SPS */
    for(i=5; i+3 <= len; i++)
        if(d[i]==0 && d[i+1]==0 && d[i+2]==1){ end = i; break; }
    if(end){ if(end > 5 && d[end-1]==0) end--;   /* that zero belongs to the next start code */ }
    else end = len;    /* no following NAL: this encoder hands the SPS over as its own
                        * ~12-byte pack, with PPS and the IDR slice in later packs */
    nlen = end - 4;
    if(nlen < 5 || nlen > SPS_MAXIN) return 0;
    *skip = end;
    if(nlen == g_spssrc_len){                    /* same SPS as last time: reuse */
        for(i=0;i<nlen;i++) if(g_spssrc[i] != d[4+i]) break;
        if(i == nlen) return g_spsfix_len;
    }
    n = sps_fix(d+4, nlen, g_spsfix+4, SPS_MAXOUT);
    for(i=0;i<nlen;i++) g_spssrc[i] = d[4+i];
    g_spssrc_len = nlen;
    g_spsfix_len = n ? n+4 : 0;
    if(!g_sps_logged){
        g_sps_logged = 1;
        puts_("[dvr] SPS aspect patch "); puts_(n ? "active\n" : "NOT applied (left as-is)\n");
    }
    return g_spsfix_len;
}
static int pump_encode(void){
    ...
    for(chn=0; chn<NCH; chn++){
        ...
        for(k=0; k<npk; k++){
            ...
            dlen = tlen - 0x40;
            { unsigned skip = 0, fl = batch_key ? sps_split(base+0x40, dlen, &skip) : 0;
              if(fl){                              /* aspect-corrected SPS, then the rest verbatim */
                  net_fanout(chn, g_spsfix, fl, batch_key);
                  net_fanout(chn, base+0x40+skip, dlen-skip, 0);
                  if(rec_on[chn] && au_len + fl + (dlen-skip) <= sizeof(au_buf)){
                      memcpy(au_buf+au_len, g_spsfix, fl); au_len += fl;
                      memcpy(au_buf+au_len, base+0x40+skip, dlen-skip); au_len += dlen-skip; }
              } else {
        ...
        if(rec_on[chn] && au_len){
            ...
            rec_last_us[chn] = pk_us;
            ts_write(&rec_ts[chn], au_buf, au_len, batch_key, pts90);
            total_bytes += au_len;
            /* Size the file by what actually LANDS in it, not the payload handed to the
             * muxer. TS framing adds 188/184 = 2.2%, so a "2000 MB" cap produced a 2045 MiB
             * file -- and sys_lseek here is 32-bit, so anything past 2047 MiB reads back a
             * negative size and playback breaks. Seen in the field at 2,144,836,164 bytes,
             * 2.6 MB from the edge. */
            rec_mb[chn] = (unsigned)(rec_ts[chn].wrote >> 20);
            ...
            else if(rec_mb[chn] >= g_max_rec_mb){
                /* ROLL OVER, do not stop. Hitting the cap used to end the recording outright,
                 * so a long flight quietly stopped being recorded partway through and nothing
                 * said so. Close this file but LEAVE rec_want set: the next keyframe opens the
                 * next numbered file, so the flight continues across 0002, 0003, ... with a
                 * sub-second seam and every file still starting on SPS/PPS/IDR. */
                puts_("[dvr] REC size cap ch"); putu((unsigned)chn);
                puts_(" -> rolling to a new file\n");
                rec_stop(chn);          /* rec_want stays set, so the next keyframe reopens */
            } }
...
/* Title for the playback bar, from either naming scheme:
 *   NNNN_chC_MMmSSs.ts  -> "#3  05m20s  CH1"     (current)
 *   YYYYMMDD_HHMMSS_chN.ts -> "MM/DD HH:MM CHn"  (legacy)
 * The numbered form went in without updating this, so the bar showed "_c/h1 05:m2 CH?" --
 * the old field offsets read straight through the new name. */
static void pb_set_title(const char *path){
    const char *n=path, *p=path; while(*p){ if(*p=='/') n=p+1; p++; }
    char *t=g_pb_title; int i=0, L=0;
    while(n[L]) L++;
    int dig4 = (n[0]>='0'&&n[0]<='9' && n[1]>='0'&&n[1]<='9' &&
                n[2]>='0'&&n[2]<='9' && n[3]>='0'&&n[3]<='9');
    if(dig4 && L>=12 && n[4]=='_' && n[5]=='c' && n[6]=='h'){       /* numbered */
        int q=0; t[i++]='#';
        while(q<3 && n[q]=='0') q++;                                 /* trim leading zeros */
        while(q<4) t[i++]=n[q++];
        t[i++]=' '; t[i++]=' ';
        { int m; for(m=8+1; m<L-3 && i<20; m++) t[i++]=n[m]; }       /* duration, or REC */
        t[i++]=' '; t[i++]=' '; t[i++]='C'; t[i++]='H';
        t[i++]=(n[7]>='1'&&n[7]<='4')?n[7]:'?';
        t[i]=0;
    } else if(dig4 && L>=22 && n[8]=='_'){                           /* legacy dated */
...
static int play_file(const char *path){
    ...
    if(vdec>=0){
        ...
        g_pb_speed=4; g_pb_seekmode=0; g_pb_seek_step=10; g_pb_seek_t=0; g_pb_frame=0; g_kf_cnt=0; g_pb_sel=PBB_PP;
        long pb_due=0;    /* running frame deadline (see the pacing block) */
        int ff_on=0; long ff_t0=0, ff_ms0=0;   /* fast-forward anchor (see below) */
        ...
        while(!g_pb_stop){
            ...
            g_pb_ended=0;
            { int iskey = (pb_au[0]==0&&pb_au[1]==0&&pb_au[2]==0&&pb_au[3]==1 &&
                           ((pb_au[4]&0x1f)==7 || (pb_au[4]&0x1f)==5));
              /* Fast-forward plays KEYFRAMES ONLY. 8x would otherwise need 240 fps of
               * decode, which this VDEC cannot do; an IDR every ~0.5 s it manages easily,
               * and each is independently decodable so the picture stays clean instead of
               * smearing. Skipping before the pacing code leaves prev_pts on the last
               * SHOWN frame, so the delta becomes the GOP interval and the existing
               * delay = dur*4/speed lands on the right wall-clock rate by itself. */
              if(g_pb_speed > 4 && !iskey && !g_pb_paused) continue; }
            ...
            if(!stepping){                                     /* real-time pace, scaled by slow-mo */
                long delay=0;
                if(have_prev){ long dur=(long)((d.pts-prev_pts)/90ULL); if(dur<0)dur=0;
                    /* Only a real discontinuity should be clamped. The old limit was 500 ms,
                     * but in fast modes we step keyframe to keyframe and a GOP is ~500 ms --
                     * every gap would have been mistaken for a glitch and paced at 33 ms. */
                    if(dur>3000) dur=33;
                    delay = dur*4/(g_pb_speed>0?g_pb_speed:4); }
                /* Fast modes are paced by the step below, not by PTS: hold each picture
                 * for a fixed beat so the display rate stays constant at any speed. */
                if(g_pb_speed > 4) delay = PB_FF_BEAT_MS;
                /* Pace against a RUNNING DEADLINE, not now+delay. Waiting the whole frame
                 * interval and THEN doing the read/demux/feed work made the real period
                 * delay+work, so 1x ran at 0.62x and 1/2x at 0.37 -- the arithmetic that
                 * gave it away, since ~20 ms of work explains both exactly. Advancing a
                 * deadline absorbs the work instead. Resync if we fall badly behind so a
                 * slow patch does not turn into a sprint afterwards. */
                if(!have_prev) pb_due = now_ms();
                pb_due += delay;
                { long n0=now_ms(); if(pb_due < n0 - 250) pb_due = n0; }
                long tend=pb_due;
                /* Wait out the frame interval WITHOUT oversleeping. This kernel ticks at
                 * 100 Hz, so the old msleep(3) actually cost ~10 ms and a 33 ms frame was
                 * routinely stretched to 40+; 1x playback measured 0.62x because of it.
                 * Only sleep while there is real time to give away, then spin the last
                 * tick out while still draining the encoder. */
                for(;;){ long now=now_ms(), left=tend-now;
                    if(left<=0 || g_pb_stop || g_pb_paused) break;
                     ...
                     * 29.97) because the VENC ring overflowed while we sat here. The input
                     * polls stay rate-limited - they are for a human. */
                    pump_encode();
            ...
            /* ---- fast-forward is SEEK-driven, not read-driven ------------------
             * Reading straight through cannot beat the demuxer: measured ~1.7x on a
             * 32 Mbps clip even with only keyframes decoded, because every byte still
             * has to be read and de-packetised. So above 1x we JUMP -- show a keyframe,
             * seek past the content we want to skip, show the next one. The step is
             * sized to keep a constant ~5 pictures/second on screen whatever the speed,
             * so the I/O per second stays flat instead of scaling with the speed. */
            if(g_pb_speed > 4 && !g_pb_paused && !stepping && g_pb_total > 0 && g_pb_tot_ms > 0){
                /* Anchor the position to the WALL CLOCK, not to a fixed increment per step.
                 * Each step costs a seek plus a scan to the next keyframe, and that cost is
                 * not constant -- stepping by a fixed amount therefore drifted slow (8x came
                 * out at 5.7x). Computing where we SHOULD be from elapsed real time makes the
                 * speed self-correcting: a slow step just means the next jump is longer. */
                if(!ff_on){ ff_on=1; ff_t0=now_ms(); ff_ms0=g_pb_cur_ms; }
                { long want = ff_ms0 + (long)((long long)(now_ms()-ff_t0) * g_pb_speed / 4);
                  if(want >= g_pb_tot_ms){ g_pb_ended=1; g_pb_paused=1; g_pb_sel=PBB_PP; ff_on=0; }
                  else {
                      long target=(long)((long long)g_pb_total * want / g_pb_tot_ms);
                      if(target > d.cur_off && target < g_pb_total-65536){
                          tsdemux_seek(&d, target); have_prev=0; }
                  } }
            } else ff_on=0;
...
int main(int argc, char **argv){
    ...
    /* Put the RTC on the kernel clock before anything timestamps anything.
     * The RTC keeps real time across power-off, but Linux boots at 1970 here and no
     * init script syncs the two (busybox on this box has no hwclock applet), so every
     * recording carried a sensible name and a 1970/1980 mtime - which is exactly what
     * made the file dates look "completely wrong" while the names looked fine. */
    kclock_from_rtc();
    g_start_sec = now_sec();
    int chn, i, r;
