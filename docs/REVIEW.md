...
## 2e. Capture geometry and display aspect — 2026-08-23

Reported after a field test as "all the recordings look stretched". They were, and it was not a
player problem: **the encoder's SPS carried no VUI at all**, so nothing in the bitstream said what
shape the picture was and every player assumed square pixels — drawing 704x240 as a 2.93:1 strip.

For H.264 there is no container-level fix; the display aspect lives only in the SPS. `sps.h` now
rewrites the SPS bit-by-bit as it leaves the encoder, inserting an Extended_SAR derived from the
dimensions the SPS itself declares. The algorithm was prototyped in Python against a captured `.ts`
and checked with ffmpeg before a line of C was written, then verified on a frame of real flight
footage: at 704x240 the OSD font is visibly squat; at 4:3 it is correctly proportioned.

**The geometry is at the hardware ceiling, and the numbers prove it hangs together:**

| | value | why it is maximal |
|---|---|---|
| width | 704 | ITU-R BT.601 defines the 4:3 picture as 704 active samples. The TW2866 emits 720; the extra 8 each side are the blanking transition, not picture. |
| height | 240 (NTSC) / 288 (PAL) | one field. `capture=1` asks the VIU for both fields woven into a 480-line frame and it returns **nothing** — VENC BUF_EMPTY, `pps=0`, re-measured 2026-08-23. The option is kept so the finding stays reproducible. |
| rate | 29.97 fps | the NTSC field rate |
| quality | `qp=10` | the minimum the config allows, ~26 Mbps, measured writing every frame |

The SAR the firmware computes is **5:11 for 704x240 and 6:11 for 704x288** — exactly half the BT.601
values of 10:11 and 12:11. Half, because we hold half the lines. That the runtime arithmetic lands
precisely on the standard values is the strongest evidence the single-field capture is being
described correctly rather than merely made to look right.
